...
struct GA[sig: Signature]:
    ...
    # +------( Multivector Parsing )------+ #
    #
    fn __getitem__[string: String](self, out result: Multivector[sig, sig.basis_mask(string)]):
        ...
        while stop < len(string):
            ...
            try:
                ...
                var value = (1 if idx_of_e == start else result.Coef(slice[:idx_of_e]))
    ...
    ](var *coefs: SIMD[type, size]) -> Multivector[sig, Self.vector_mask, type, size]:
    ...
    fn bivector[
        type: DType = DType.float64, size: Int = 1
    ](var *coefs: SIMD[type, size]) -> Multivector[sig, Self.bivector_mask, type, size]:
