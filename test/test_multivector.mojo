...
def test_subspace_constructor() raises:
    ...
    assert_true(G3.bivector(4, 5, 6) == G3.Multivector[G3.bivector_mask](4, 5, 6))
    assert_true(G3.bivector(4, 0, 6) != G3.Multivector[G3.bivector_mask](4, 5, 6))
...
def test_mul() raises:
    ...
    comptime ug3 = Signature(1, 1, 1, flip_ze=False)
    comptime v1_mask = BasisMask(Basis(bin=0b001))
    assert_true(Multivector[ug3, v1_mask](2).__mul__(Multivector[ug3, v1_mask](2)) == Float64(4))
    comptime v2_mask = BasisMask(Basis(bin=0b010))
    assert_true(Multivector[ug3, v2_mask](2).__mul__(Multivector[ug3, v2_mask](2)) == Float64(-4))
    comptime v3_mask = BasisMask(Basis(bin=0b100))
    assert_true(Multivector[ug3, v3_mask](2).__mul__(Multivector[ug3, v3_mask](2)) == Float64(0))

    comptime niltrivector_mask = BasisMask(Basis(bin=0b0111), Basis(bin=0b1011), Basis(bin=0b1101))
    assert_true(PG3.Multivector[PG3.vector_mask](1, 2, 3, 4).__mul__(Float64(2)) == PG3.Multivector[PG3.vector_mask](2, 4, 6, 8))
